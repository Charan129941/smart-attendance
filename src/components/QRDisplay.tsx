...
export default function QRDisplay({ qrDataUrl, attendanceUrl, expiresAt, onRotate, intervalSecs }: QRDisplayProps) {
  ...
  return (
    <>
      ...
      {isFullscreen && (
        <div className="fixed inset-0 z-50 flex flex-col items-center justify-center p-8 bg-[#0a0f1e]">
          <button 
          ...
          <h2 className="text-4xl font-bold text-white mb-2">Scan to Mark Attendance</h2>
          <p className="text-xl text-gray-400 mb-12">Please stay in your seats. The code refreshes automatically.</p>
          
          <div className="bg-white p-8 rounded-3xl shadow-[0_0_50px_rgba(59,130,246,0.3)] mb-12">
            {qrDataUrl ? (
              <img src={qrDataUrl} alt="Attendance QR Code" className="w-[500px] h-[500px] object-contain" />
            ) : (
              <div className="w-[500px] h-[500px] flex items-center justify-center text-gray-400 text-2xl">
                Generating...
              </div>
            )}
          </div>
